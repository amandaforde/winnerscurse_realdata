...
## plot diff
plot(diff, main="BMI GWAS 1", col.main=col[1]) + abline(h=0)
```
...
## plot diff bmi 2
plot(diff_bmi_2, main="BMI GWAS 2", col.main = col[2]) + abline(h=0)

## plot diff bmi 2
plot(diff_T2D_1, main="T2D GWAS 1", col.main = col[3]) + abline(h=0)

## plot diff bmi 2
plot(diff_T2D_2, main ="T2D GWAS 2", col.main = col[4]) + abline(h=0)
...
Thus, based on this empirical analysis of real data sets, it seems appropriate to suggest using a pre-specificed value of 6 for the degrees of freedom. In `winnerscurse` package, using `èmpirical_bayes()` we can set `AIC=FALSE`. Below, we will see what this adaptation means for us in terms of our chosen evaluation metrics. 
...
Here, we see that our three suggested improvements on the original empirical Bayes method have performed better, with the one in which we have fixed the degrees of freedom to 6 having the lowest `mse` value. We note that the adaptation to FDR IQT is performing worse than it does in its original form. This is most likely due to some SNPs undergoing over-reductions in their estimated effect sizes.
...
These results are very similar to those above but with `empirical_bayes_scam` performing slightly better this time. 
...
We witness similar results to those for **BMI GWAS 1**. Both adaptations have improved the empirical Bayes method, but with the fixing of the degrees of freedom being the clear winner here with the lowest value. Also, the change to FDR IQT continues to perform poorly in comparison to the original definition of the method in which it doesn't adjust the estimated effect sizes of SNPs with very small $p$-values, i.e. < `1e-15`.
...
This second set of results support the observations we have made above. 

...
The above modification to the empirical Bayes method can be seen to work well as it enforces additional constraints on the shape of the estimated log density function. Thus, similar functions which impose constraints on the shape may also perform well. Thus, we could also assess a variation of the empirical Bayes method in which the `gam` function in the R package `mgcv` is employed. The `gam` function fits a generalized additive model (GAM), with smoothness estimation integrated in the fitting process. The smoothing parameters are selected by means of generalized cross-validation (GCV). Two uses of this function were investigated, one in which the distribution family was specified as poisson and the other in which the identification of the family as negative binomial took place.

The function definition remains extremely similar to that above but with the following lines
...
As before, we will first apply a threshold of `5e-8` in order to classify significant SNPs and subsequently, a threshold of `5e-4`.
...
In addition to our detailed exploration of method performance above, we decided to investigate a third real data set, namely height. We followed the exact same procedure as the one used with the BMI data set illustrated previously. 

The original data set contained height values for a total of 502,554 individuals. The pre-processing steps and implementation of `split_height.R` resulted in two independent data sets. The first, namely `height_gwasA.txt`, contained 166,250 individuals for which genotype data was also available while the second, namely `height_gwasB.txt` contained 166,687. 

Using PLINK 2.0, we performed both GWASs and obtained two sets of summary statistics for a total of 7,915,560 SNPs. We will refer to the instance in which `height_gwasA.txt` has been used to conduct the discovery GWAS and `height_gwasB.txt` has been used for the replication GWAS as **Height GWAS 1**. Similarly, we will refer to the case in which `height_gwasB.txt` has been used for the discovery GWAS and `height_gwasA.txt` for the replication as **Height GWAS 2**. 

In **Height GWAS 1**, we find that out of the 70,020 significant SNPs at `5e-8`, 45,119 of these SNPs have an association estimate which is more extreme in the discovery GWAS than in the replication. We find that 12,809 SNPs have *significantly* more extreme association estimates in the discovery GWAS. There are 257,000 SNPs which are significant at the larger`5e-4` threshold. 
...
In **Height GWAS 2**, we find that out of the 70,634 significant SNPs at `5e-8`, 48,205 of these SNPs have an association estimate which is more extreme in the discovery GWAS than in the replication. We find that 12,514 SNPs have *significantly* more extreme association estimates in the discovery GWAS. There are 268,497 SNPs which are significant at the larger`5e-4` threshold.
...
Similar to the other two traits, we notice in one GWAS that many SNPs with large $z$-values, in this case greater than 15, are seen to have association estimates in the replication GWAS which are greater than their corresponding association estimates in the discovery GWAS. Again, we hypothesize that LD is responsible for this as it is most likely that these SNPs all represent one independent signal, i.e. they are not independent from one another.  


We find there to be 47,324 SNPs which are in both sets of significant SNPs at the `5e-8` threshold, while there are 154,389 SNPs in both sets at the `5e-4` threshold.  
...
We begin by applying the methods, including the alternative forms of FDR IQT and empirical Bayes discussed above, and use a threshold of `5e-8` in order to classify significant SNPs. For **Height GWAS 1**, we obtain the following adjusted association estimates for the top 6 significant SNPs.
...
We see that the empirical Bayes method in the one in which performs the best at producing bias-reduced association estimates for the SNPs which have been deemed significant at this threshold. We note how all three conditional likelihood methods substantially increase `mse` compared to the `naive` approach. This is concrete evidence that supports the suggestion that conditional likelihood methods should not be used to make Winner's Curse adjustments to estimated effect sizes of significant SNPs. Concerningly, we see that both of our suggested improvements on the original empirical Bayes method have performed worse, with the one in which we have fixed the degrees of freedom to 7 having a larger `mse` value than that of the `naive` approach. We note that contrary to previous observations the adaptation to FDR IQT is performing better than it does in its original form.
...
We witness a similar pattern to that above but with most methods having lower values than that of the `naive` approach. It is the bootstrap method which performs the best in this instance. These results with respect to the comparison between the original empirical Bayes method and its suggested modifications are very similar to those above, in which we can draw the same conclusions. 

<br>

#### Height 2

We repeat the above process for **Height GWAS 2**. Here, we keep in mind what we observed in our $z$ vs $\text{bias}$ plot for **Height GWAS 2** - we saw that for the most significant SNPs to the right of the plot, their association estimates in the replication GWAS were in fact *larger* than in the discovery GWAS. 
...
Here, all the conditional likelihood methods have values larger than the `naive` approach while the empirical Bayes method with the inclusion of the shape-constrained additive model is seen to perform the best overall. Unfortunately, the adaptation of the empirical Bayes method which fixes the degrees of freedom does not show an improvement and in fact results in worse performance than the FDR IQT and bootstrap methods as well. The change to FDR IQT seems to work slightly better here than the original definition of the method in which it doesn't adjust the estimated effect sizes of SNPs with very small $p$-values, i.e. < `1e-15`.
...
Again, we witness a similar pattern to **Height GWAS 1** at this threshold of `5e-4` in which all methods perform better than the `naive` approach. The values for the original empirical Bayes method and FDR IQT are extremely similar. The empirical Bayes method which has made use of the `scam()` function has the lowest value here. 
...
The results from comparing all mentioned forms of the different Winner's Curse correction methods can be summarised in a visual manner with the plots below. 
